...
# Definition of FormatText was moved to EOF.
# Definition of LoadText was moved to EOF.
# Definition of GetExtension was moved to EOF.
...
########################################################### template PIXELFORMAT_UNCOMPRESSED_R8G8B8A8*(): auto = PIXELFORMAT_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 ######################
# ----------------------------------------------------------------------------------
...
# Definition of SpriteFont was moved to EOF.
...
type MeshVertexAttributes* = enum 
...
converter MeshVertexAttributes2int32* (self: MeshVertexAttributes): int32 = self.int32 
# Material map index
...
converter Gestures2int32* (self: Gestures): int32 = self.int32 
# Camera system modes
...
type bytesRead* = cuchar
type LoadFileTextCallback* = char
# ------------------------------------------------------------------------------------
...
proc GetFrameTime*(): float32 {.RLAPI, importc: "GetFrameTime".} # Returns time in seconds for last frame drawn (delta time)
...
######################################proc SetTraceLogCallback*(callback: int) {.RLAPI, importc: "SetTraceLogCallback".} # Set custom trace log
...
proc UnloadFileText*(text: uint8) {.RLAPI, importc: "UnloadFileText".} # Unload file text data allocated by LoadFileText()
...
proc GetMouseWheelMove*(): float32 {.RLAPI, importc: "GetMouseWheelMove".} # Returns mouse wheel movement Y
...
# ------------------------------------------------------------------------------------
proc InitVrSimulator*(device: VrDeviceInfo) {.RLAPI, importc: "InitVrSimulator".} # Init VR simulator for selected device parameters
...
proc BeginVrDrawing*(target: RenderTexture2D) {.RLAPI, importc: "BeginVrDrawing".} # Begin VR simulator stereo rendering (using provided fbo)
proc EndVrDrawing*() {.RLAPI, importc: "EndVrDrawing".} # End VR simulator stereo rendering
proc GetVrConfig*(device: VrDeviceInfo): VrStereoConfig {.RLAPI, importc: "GetVrConfig".} # Get stereo rendering configuration parameters
# ------------------------------------------------------------------------------------
...
proc ColorFromHSV*(hue: float32; saturation: float32; value: float32): Color {.RLAPI, importc: "ColorFromHSV".} # Returns a Color from HSV values
...
proc UploadMesh*(mesh: ptr Mesh; dynamic: bool) {.RLAPI, importc: "UploadMesh".} # Upload vertex data into GPU and provided VAO/VBO ids
...
proc UnloadModelAnimations*(animations: ModelAnimation, count: uint32) {.RLAPI, importc: "UnloadModelAnimations".} # Unload animation array data
...
proc GenMeshCustom*(vertexCount: int32; flags: int32): Mesh {.RLAPI, importc: "GenMeshCustom".} # Generate custom empty mesh (data initialized to 0)
...
proc DrawBillboardRec*(camera: Camera; texture: Texture2D; source: Rectangle; center: Vector3; size: float32; tint: Color) {.RLAPI, importc: "DrawBillboardRec".} # Draw a billboard texture defined by source
# Collision detection functions
proc CheckCollisionSpheres*(center1: Vector3; radius1: float32; center2: Vector3; radius2: float32): bool {.RLAPI, importc: "CheckCollisionSpheres".} # Detect collision between two spheres
...
proc GetCollisionRayMesh*(ray: Ray; mesh: Mesh; transform: Matrix): RayHitInfo {.RLAPI, importc: "GetCollisionRayMesh".} # Get collision info between ray and mesh
...
proc SetShaderValueTexture*(shader: Shader; locIndex: int32; texture: Texture2D) {.RLAPI, importc: "SetShaderValueTexture".} # Set shader uniform value for texture
# ------------------------------------------------------------------------------------
...
proc LoadMusicStreamFromMemory*(fileType: cstring; data: cuchar, dataSize: int32): Music {.RLAPI, importc: "LoadMusicStreamFromMemory".} # Load module music from data
